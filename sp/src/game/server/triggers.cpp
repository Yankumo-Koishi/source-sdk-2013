...
//-----------------------------------------------------------------------------
// Purpose: Render Portal 2's SolidEnergy field and draw vortex if something
// is near (player, npc or prop) 
//-----------------------------------------------------------------------------
class CTriggerField : public CBaseTrigger
{
	public:
		DECLARE_CLASS(CTriggerField, CBaseTrigger);
		DECLARE_DATADESC();
...
BEGIN_DATADESC(CTriggerField)
	DEFINE_KEYFIELD(m_fWakeSpeed, FIELD_FLOAT, "wakeSpeed"),
	DEFINE_KEYFIELD(m_fSleepSpeed, FIELD_FLOAT, "sleepSpeed"),
	DEFINE_KEYFIELD(m_fSleepSpeed, FIELD_STRING, "sleepSpeed"),
END_DATADESC()

void CTriggerField::Spawn()
{
	BaseClass::Spawn();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriggerField::Think(void)
{	
	modelinfo->GetModelMaterials(const_cast<model_t*>(GetModel()), 1, &pMaterial);
	
	IMaterialVar* pVortexPos1 = pMaterial->FindVar("$FLOW_VORTEX_POS1", NULL, false);
	IMaterialVar* pVortex1 = pMaterial->FindVar("$FLOW_VORTEX1", NULL, false);
	IMaterialVar* pVortexPos2 = pMaterial->FindVar("$FLOW_VORTEX_POS2", NULL, false);
	IMaterialVar* pVortex2 = pMaterial->FindVar("$FLOW_VORTEX2", NULL, false);
	IMaterialVar* alpha = pMaterial->FindVar("$powerup", NULL, false);

	// Update cached state to update time when fade is started
	if (m_State != !m_bDisabled)
	{
		m_fFadeTime = gpGlobals->curtime;
		m_State = !m_bDisabled;
	}

	// Update alpha only when necessary
	if (alpha->GetFloatValue() != m_fWakeTargetAlpha)
		alpha->SetFloatValue(m_fWakeTargetAlpha);

	// Fade field on disable/enable
	float frac = (gpGlobals->curtime - m_fFadeTime);

	if (m_bDisabled)
		m_fWakeTargetAlpha = Lerp(clamp(frac / m_fSleepSpeed, 0.f, 1.f), m_fWakeTargetAlpha, 0.0f);
	else
		m_fWakeTargetAlpha = Lerp(clamp(frac / m_fWakeSpeed, 0.f, 1.f), m_fWakeTargetAlpha, 1.0f);

	// Get min/max of trigger bounds
	Vector vecSurroundMins, vecSurroundMaxs, vecLocalCenter;
	CollisionProp()->WorldSpaceSurroundingBounds(&vecSurroundMins, &vecSurroundMaxs);

	// Get center of trigger
	VectorAdd(vecSurroundMins, vecSurroundMaxs, vecLocalCenter);
	vecLocalCenter *= 0.5f;

	// Find all nearby entities (in 64 units)
	CBaseEntity* pEntities[2];
	int nEntCount = UTIL_EntitiesInSphere(pEntities, 2, vecLocalCenter, 64, 0);

	// Actual count of needed entities
	int actualCount = 0;
	...
	if (nEntCount >= 0)
	{
		...
		{
			CBaseEntity* ent = pEntities[i];
			Vector pos = ent->GetAbsOrigin();

			// Ignore unvalid entities
			if (ent == NULL)
				continue;
			...
			if ((!Q_stricmp(ent->GetClassname(), "prop_physics") && HasSpawnFlags(16384)) || (ent->IsNPC() && HasSpawnFlags(32768)) || (ent->IsPlayer() && HasSpawnFlags(65536)))
			{
				actualCount++;

				// Now only those entities make vortex
				if (i == 0)
				{
					pVortex1->SetIntValue(1);
					pVortexPos1->SetVecValue(pos.x, pos.y, pos.z + 32);
				}
				else
				{
					pVortex2->SetIntValue(1);
					pVortexPos2->SetVecValue(pos.x, pos.y, pos.z + 32);
				}
			}
		}
	}

	// If no any entities found (based on coniditions) we stop rendering vortex
	if (actualCount == 0)
	{
		if (pVortex1->GetIntValue() > 0)
			pVortex1->SetIntValue(0);

		if (pVortex2->GetIntValue() > 0)
			pVortex2->SetIntValue(0);
	}
