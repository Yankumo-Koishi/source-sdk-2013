...
#endif
			{
				DECLARE_STATIC_VERTEX_SHADER_SCELL( solidenergy_vs20 );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( VERTEXCOLOR,  bHasVertexColor || bHasVertexAlpha );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( DETAIL1, bDetail1 );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( DETAIL2, bDetail2 );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( TANGENTTOPACITY, bTangentT );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( TANGENTSOPACITY, bTangentS );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( FRESNELOPACITY, bFresnel );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( FLOWMAP, bHasFlowmap );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( MODELFORMAT, bModel );
				SET_STATIC_VERTEX_SHADER_SCELL( solidenergy_vs20 );
			}
#if !defined( _GAMECONSOLE )
			else
			{
				DECLARE_STATIC_VERTEX_SHADER_SCELL( solidenergy_vs30 );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( VERTEXCOLOR,  bHasVertexColor || bHasVertexAlpha );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( DETAIL1, bDetail1 );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( DETAIL2, bDetail2 );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( TANGENTTOPACITY, bTangentT );
				...
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( FLOWMAP, bHasFlowmap );
				SET_STATIC_VERTEX_SHADER_COMBO_SCELL( MODELFORMAT, bModel );
				SET_STATIC_VERTEX_SHADER_SCELL( solidenergy_vs30 );
			}
#endif
...
#endif
			{
				DECLARE_STATIC_PIXEL_SHADER_SCELL( solidenergy_ps20b );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( ADDITIVE, bAdditiveBlend );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DETAIL1, bDetail1 );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DETAIL2, bDetail2 );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DETAIL1BLENDMODE, nDetail1BlendMode );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DETAIL2BLENDMODE, nDetail2BlendMode );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( VERTEXCOLOR, ( bHasVertexColor || bHasVertexAlpha ) );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( TANGENTTOPACITY, bTangentT );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( TANGENTSOPACITY, bTangentS );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( FRESNELOPACITY, bFresnel );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DEPTHBLEND, bDepthBlend );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( FLOW_CHEAP, bHasCheapFlow );
				SET_STATIC_PIXEL_SHADER_SCELL( solidenergy_ps20b );
			}
#if !defined( _GAMECONSOLE )
			else
			{
				DECLARE_STATIC_PIXEL_SHADER_SCELL( solidenergy_ps30 );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( ADDITIVE, bAdditiveBlend );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DETAIL1, bDetail1 );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DETAIL2, bDetail2 );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( DETAIL1BLENDMODE, nDetail1BlendMode );
				...
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO_SCELL( FLOW_CHEAP, bHasCheapFlow );
				SET_STATIC_PIXEL_SHADER_SCELL( solidenergy_ps30 );
			}
#endif
...
#ifndef _PS3
			pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant( 4, params[info.m_nDepthBlendScale]->GetFloatValue() );
#endif
...
#endif
		{
			DECLARE_DYNAMIC_VERTEX_SHADER_SCELL( solidenergy_vs20 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( COMPRESSED_VERTS, (int)vertexCompression );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( SKINNING,  pShaderAPI->GetCurrentNumBones() > 0 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( VORTEX1, bVortex1 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( VORTEX2, bVortex2 );
			SET_DYNAMIC_VERTEX_SHADER_SCELL( solidenergy_vs20 );
		}
#if !defined( _GAMECONSOLE )
		else
		{
			DECLARE_DYNAMIC_VERTEX_SHADER_SCELL( solidenergy_vs30 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( COMPRESSED_VERTS, (int)vertexCompression );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( SKINNING,  pShaderAPI->GetCurrentNumBones() > 0 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( VORTEX1, bVortex1 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_SCELL( VORTEX2, bVortex2 );
			SET_DYNAMIC_VERTEX_SHADER_SCELL( solidenergy_vs30 );
		}
#endif
...
#endif
		{
			DECLARE_DYNAMIC_PIXEL_SHADER_SCELL( solidenergy_ps20b );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( ACTIVE, bActive );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( POWERUP, bPowerup );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( VORTEX1, bVortex1 );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( VORTEX2, bVortex2 );
			SET_DYNAMIC_PIXEL_SHADER_SCELL( solidenergy_ps20b );
		}
#if !defined( _GAMECONSOLE )
		else
		{
			DECLARE_DYNAMIC_PIXEL_SHADER_SCELL( solidenergy_ps30 );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( ACTIVE, bActive );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( POWERUP, bPowerup );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( VORTEX1, bVortex1 );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_SCELL( VORTEX2, bVortex2 );
			SET_DYNAMIC_PIXEL_SHADER_SCELL( solidenergy_ps30 );
		}
#endif

		if ( bDetail1 )
		{
			DynamicCmdsOut.BindTexture( pShader, SHADER_SAMPLER1, info.m_nDetail1Texture, info.m_nDetail1Frame );
		}

		if ( bDetail2 )
		{
			DynamicCmdsOut.BindTexture( pShader, SHADER_SAMPLER4, info.m_nDetail2Texture, info.m_nDetail2Frame );
		}

		if ( bDepthBlend )
		{
			DynamicCmdsOut.BindStandardTexture( SHADER_SAMPLER3, TEXTURE_FRAME_BUFFER_FULL_DEPTH );
		}
